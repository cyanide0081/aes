...
table_index :: proc(t: table, index: u8) -> u8 {
	lsn := u8(index & 0x0F)
	msn := u8((index >> 4) & 0x0F)
	return t[msn][lsn]
}
...
galois_mul :: proc(a, b: u8) -> u8 {
	if a == 0 || b == 0 {
		...
		a_l := table_index(GALOIS_L, a)
		b_l := table_index(GALOIS_L, b)
		l_sum := galois_add(a_l, b_l)
		return table_index(GALOIS_E, l_sum)
	}
}

...
encrypt :: proc(
	...
	for i in 0..<block_count {
		...
		if args.mode == .CBC {
			block = mat_xor(block, chain)
		}

		block = mat_xor(block, key_schedule[0])
		...
		for j in 1..<10 {
			...
			block = mat_xor(mix_cols, key_schedule[j])
		}

		sub_bytes := mat_sub_bytes(block, S_BOX_ENC)
		shift_rows := mat_shift_rows(sub_bytes, SHIFT_ROWS_ENC)
		block = mat_xor(shift_rows, key_schedule[10])
...
word_xor :: proc(a, b: word) -> word {
	a, b := transmute(u32)a, transmute(u32)b
	return transmute(word)(a ~ b)
}

word_sub_bytes :: proc(w: word, table: [16][16]u8) -> (sub_word: word) {
	for byte, i in w {
		sub_word[i] = sub_table_index(table, byte)
	}

	return
}

// most significant nibble -> row, least significant nibble -> column
sub_table_index :: proc(table: [16][16]u8, index: u8) -> u8 {
	lsn := u8(index & 0x0F)
	msn := u8((index >> 4) & 0x0F)
	return table[msn][lsn]
}
...
mat_xor :: proc(a, b: mat4) -> mat4 {
	a, b := transmute(u128)a, transmute(u128)b
	return transmute(mat4)(a ~ b)
}
...
mat_sub_bytes :: proc(mat: mat4, table: [16][16]u8) -> (result: mat4) {
	for i in 0..<4 {
		result[i] = word_sub_bytes(mat[i], table)
	}

	return
}

// NOTE: galois multiply tables
GALOIS_L := [16][16]u8{
...
galois_mul :: proc(a, b: u8) -> u8 {
	if a == 0 || b == 0 {
		...
		a_l := sub_table_index(GALOIS_L, a)
		b_l := sub_table_index(GALOIS_L, b)
		l_sum := galois_add(a_l, b_l)
		return sub_table_index(GALOIS_E, l_sum)
	}
}
